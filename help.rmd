...
|  Argument | Selects  |  Example |
|--------|-------------------------|-----------------------|
|  subset    | individuals  | subset = covariates(ch) == "M" |
|  occasions | sampling occasions | occasions = 1:3 |
|  traps     | detectors  | |
|  sessions  | sessions   | session = 1 |

Each argument is a vector of subscripts that may be logical- (length equal to the relevant dimension of ch), character- or integer-valued, except that occasions should be specified by their number.

Selecting individuals from multi-session data is problematic. Otherwise, criteria may be combined.

The Summary table includes the filter text unless the field is deselected.
